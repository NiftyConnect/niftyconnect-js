...
import { MerkleValidatorAbi } from '../abis';
import { PAYMENT_CONTRACT_SYMBOL_MAPPING, ZERO_ADDRESS } from '../constants';

import { NiftyOrderType, Side, Network, TokenStandard } from '../types';
...
export const buildCalldata = ({
  ...
  if (tokenStandard === TokenStandard.erc1155) {
    ...
    if (paymentToken === ZERO_ADDRESS) {
      return iface.encodeFunctionData('matchERC721UsingCriteria', [
        from,
        to,
        nftAddress,
        tokenId,
      ...
      return iface.encodeFunctionData(
        ...
        [from, to, nftAddress, tokenId, merkleRoot, []]
      );
    }
  }
};
...
export const generateSellReplacementPatternForNormalOrder = ({
  tokenStandard,
}: {
  tokenStandard?: TokenStandard;
}) => {
...
export const generateBuyReplacementPatternForNormalOrder = ({
  tokenStandard,
}: {
  tokenStandard?: TokenStandard;
}) => {
...
export const getReplacementPattern = ({
  ...
  if (side === Side.Sell) {
    if (orderType === 'Normal') {
      ...
      return generateSellReplacementPatternForCollectionBasedOrder({
        tokenStandard,
      });
    } else {
      return generateSellReplacementPatternForTraitBasedOrder({
        tokenStandard,
      });
    }
  } else {
    if (orderType === 'Normal') {
      ...
      return generateBuyReplacementPatternForCollectionBasedOrder({
        tokenStandard,
      });
    } else {
